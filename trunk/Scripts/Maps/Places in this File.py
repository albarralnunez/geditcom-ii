...
cbox = None
if cname == "Africa" :
    cbox = [-46.900452,-25.35874,37.56712,63.525379]
elif cname == "Asia" :
    cbox = [-12.56111,19.6381,82.50045,180]
elif cname == "Antarctic" :
    cbox = [-90,-180,-53.00774,180]
elif cname == "Australia" :
    cbox = [-53.05872,105.377037,-6.06945,-175.292496]
elif cname == "Europe" :
    cbox = [27.636311,-31.266001,81.008797,39.869301]
elif cname == "North America" :
    cbox = [5.49955,-167.276413,83.162102,-52.23304]
elif cname == "South America" :
    cbox = [-59.450451,-109.47493,13.39029,-26.33247]
...
for i in range(numRecs) :
    ...
    lon = bb[1].coordinate
    if cbox!=None :
        if lat<cbox[0] or lat>cbox[2] : continue
        if cname == "Australia" :
            if lon<cbox[1] and lon>cbox[3] : continue
        else :
            if lon<cbox[1] or lon>cbox[3] : continue
    
    # see if need to expand bounding box
    if bbox==None :
        if lon<0. and cname=="Australia" :
            bbox = [lat-.1,lon+359.9,lat+.1,lon+360.1]
        else :
        ...
        if lon<0. and cname=="Australia" :
            if lon+360. < bbox[1] : bbox[1] = lon+360.
