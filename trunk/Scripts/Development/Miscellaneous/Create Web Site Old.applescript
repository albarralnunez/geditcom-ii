...
property eventName : {"Baptism", "Christening", "Census", "Emigration", "Immigration", "Naturalization", "Burial"}
property eventVerb : {"was baptized", "was christened", "is recorded in the census done", "emigrated", "immigrated", "was naturalized", "was buried"}
...
global notesOption -- decide which notes to include
global mmOption -- decide which multimedia to include
...
on ExportPage(indiRec)
	...
	*)
	...
	if CurrentPrivate is true then
		...
		-- birth and death
		set etext to GetTextForEvent(indiRec, "BIRT", "was born", pref, "")
		...
		set LastPlace to ""
		set etext to GetTextForEvent(indiRec, "DEAT", "died", pref, "")
		if etext is not "" then
			set pref to ""
			set end of itext to etext
			if last character of etext is return then
	...
	if CurrentPrivate is false then
		...
		if attrs is not "" then
			...
			set end of itext to "<tr><th>Attribute</th><th>Details</th></tr>" & return
	...
	tell application "GEDitCOM II"
		...
		repeat with i from 1 to numEvnt
			...
			tell evnt
				...
				if ed is "" then
					set ed to HeSheNow & "  " & everb & ", but date and place are unknown. "
				end if
				...
				-- get notes
				if notesOption is not "hideAll" then
...
on GetMarriages(indiRec)
	...
	tell application "GEDitCOM II"
		...
		set mtext to "<p>"
		if nmr > 1 then
			set mtext to mtext & HeSheNow & " had " & nmr & " marriages/partners. "
			set HeSheNow to HeShe
		...
		repeat with m from 1 to nmr
			...
			if CurrentPrivate is false then
				if umr is not "Y" then
					set HeSheNow to "They"
					set mtext to mtext & my GetTextForEvent(famRec, "MARR", "were married", "", "")
			...
			if nch = 0 then
				...
				if nch = 1 then
					set mtext to mtext & "Their known child was "
				else
...
on GetPlus(childRef)
	...
	if number of items in famc > 0 then
...
on GetMainNotes(indiRec)
	if notesOption is "hideAll" then return ""
	set nts to ""
	tell application "GEDitCOM II"
		tell indiRec
		...
		repeat with mm from 1 to numobje
			...
			if mmNames is not "" then
				...
				if mmForm is "pdf" then
					set mmOut to mmOut & "<td><a href='../media/" & mmName & "' target='_blank'>"
				else
...
(* if file is a supported file type, copy it to the media folder using a unique name
...
on CopyOBJEFile(objeRef, indiRec)
	tell application "GEDitCOM II"
		...
		if mmLoc is not "" or theDate is not "" then
			set mmtext to mmtext & "<p>The multimedia object was in "
			if mmLoc is "" then
