(*	Create Web Site script for GEDitCOM II
	...
	Latest Revision 02 Jul 2010
*)

property scriptName : "Create Web Site"
property rootName : "GC Site"
property eventOut : {"BIRT", "DEAT", "BAPM", "CHR", "CENS", "EMMI", "IMMI", "NATU", "BURI","RESI"}
property eventName : {"Birth", "Death", "Baptism", "Christening", "Census", "Emigration", "Immigration", "Naturalization", "Burial","Residence"}
...
on GetLinkTo(theRec, pref)
	tell application "GEDitCOM II"
		...
		set iname to format name value iname case "title"
		set iname to name parts gedcom name iname
		try
			set iname to word 1 of (item 1 of iname) & " " & item 2 of iname & " " & item 3 of iname
...
on GetChilLinkTo(theRec, pref)
	tell application "GEDitCOM II"
		...
		tell theRec
			...
			set bdate to evaluate expression "BIRT.DATE"
			set ddate to evaluate expression "DEAT.DATE"
			if bdate is not "" then
				set bdate to date parts full date (bdate)
				set bpref to item 1 of bdate
				set bdate to item 2 of bdate
				set b to length of bdate
				set bdate to characters (b - 3) through b of bdate
			end if
			if ddate is not "" then
				set ddate to date parts full date (ddate)
				set dpref to item 1 of ddate
				set ddate to item 2 of ddate
				set d to length of ddate
				set ddate to characters (d - 3) through d of ddate
			end if
			if bdate is not "" or ddate is not "" then
				set iname to iname & " ("
				if bdate is "" then
					set iname to iname & "?-"
				else
					set iname to iname & bpref & bdate & "-"
				end if
				if ddate is "" then
					set iname to iname & "?"
				else
					set iname to iname & dpref & ddate
				end if
				set iname to iname & ")"
			end if
			--set iname to iname & " x"
		end tell
	end tell
	return "<a href='" & pref & fname & "'>" & iname & "</a>"
end GetChilLinkTo
...
on ExportPage(indiRec)
	...
	-- Before getting details set note counter to 0
	set NoteNum to 0
		...
		tell application "GEDitCOM II"
			tell indiRec
				...
				if numBirt = 0 then
					...
					tell BirtRef
						set bdate to date parts full date (event date user)
						if item 1 of bdate is "ABT" or item 1 of bdate is "EST" or item 1 of bdate is "INT" or item 1 of bdate is "c" then
							set bdate to "about " & item 2 of bdate
							set dprep to " in"
						else if item 1 of bdate is "BEF" then
							set bdate to "before " & item 2 of bdate
							set dprep to ""
						else if item 1 of bdate is "AFT" then
							set bdate to "after " & item 2 of bdate
							set dprep to ""
						else
							set bdate to event date user
						end if
				...
				if numDeat = 0 then
					...
					tell DeathRef
						set ddate to date parts full date (event date user)
						
						if ddate is not "" then
							if item 1 of (sdn range full date (event date user)) = item 2 of (sdn range full date (event date user)) then
								set dprep to " on"
							else
								set dprep to " in"
							end if
						...
						if item 1 of ddate is "ABT" or item 1 of ddate is "EST" or item 1 of ddate is "INT" or item 1 of ddate is "c" then
							set ddate to "about " & item 2 of ddate
							set dprep to " in"
						else if item 1 of ddate is "BEF" then
							set ddate to "before " & item 2 of ddate
							set dprep to ""
						else if item 1 of ddate is "AFT" then
							set ddate to "after " & item 2 of ddate
							set dprep to ""
						else
							set ddate to event date user
						end if
	...
	tell application "GEDitCOM II"
		...
		repeat with i from 1 to numAttrs
			...
			if attrText is not "" then
				...
				set attrAll to attrAll & "<td>" & return
				set attrAll to attrAll & my GetSourceDetail(attr)
				set attrAll to attrAll & "</td>" & return
				set attrAll to attrAll & "<td>" & return
...
on GetRowForEvnt(theRec, theTag, theName)
	tell application "GEDitCOM II"
		...
		repeat with i from 1 to numEvnts
			...
			set evntAll to evntAll & "<td>" & return
			set evntAll to evntAll & my GetSourceDetail(evnt)
			set evntAll to evntAll & "</td>" & return
			set evntAll to evntAll & "<td>" & return
	...
	tell application "GEDitCOM II"
		tell sour
			...
			repeat with s from 1 to snum
				...
				set stitl to evaluate expression "SOUR.i." & s & ".TITL"
				--	display dialog id of (contents of srec) 
				...
				set sData to sData & "<a class='sour'>" & stitl & "<span>" & return
				...
				if sdate is not "" then
					set sData to sData & "<tr><td align='right'>Date: </td><td>" & sdate & "</td></tr>" & return
				end if
				...
				set sData to sData & "</span></a><br>" & return
...
on GetMarriages(indiRec)
	...
	tell application "GEDitCOM II"
		...
		if nmr > 1 then
			...
			set mtext to mtext & HeShe & " had " & item nmr of NumbsCard & " marriages/partners. "
			set HeSheNow to HeShe
			set HisHerNow to HisHer
			set ordn to " " & item nmr of NumbsOrd & " "
		else
			set ordn to " "
		end if
		...
		repeat with m from 1 to nmr
			...
			if nmr ³ 2 then
				set ordn to " " & item m of NumbsOrd & " "
			end if
...
on GetMainNotes(indiRec)
	...
	tell application "GEDitCOM II"
			...
			if number of items in noteData is 2 then
				...
				tell note id noteID of front document
					...
					if thenote is not "" then
						...
						if nn > 1 then
							set nts to nts & "<p><h4><p><a name='" & nnum & "'></a><p><b>Note " & nnum & ". </b></h4></p>" & htmlNote & return
						else
		...
		repeat with n from (nnum + 1) to NoteNum
...
on GetEventAttrNotes(struct)
	tell application "GEDitCOM II"
		...
		if NoteNum = 0 then
			tell indiRec
				set theNotes to find structures tag "NOTE" output "references"
			end tell
			
			set nn to number of items in theNotes
			repeat with i from 1 to nn
				set noteData to item i of theNotes
				if level of noteData is 1 then
					set NoteNum to NoteNum + 1
					set end of evntAttrNotes to " "
				end if
			end repeat
		end if
		...
		repeat with i from 1 to nn
			...
			tell note id noteID of front document
				...
				if thenote is not "" then
					...
					set nts to nts & "<p><a href='#Note" & NoteNum & "'> See Note " & NoteNum & ".</a></p>"
				end if
			end tell
			--end if
		end repeat
		...
		repeat with i from 1 to nn
			...
			tell note id noteID of front document
				...
				if thenote is not "" then
					...
					if i = 1 then
						set nts to nts & "<sup>[Note"
						if nn > 1 then
							set nts to nts & "s"
						end if
					end if
					set nts to nts & "<a href='#Note" & NoteNum & "'><small>"
					
					set nts to nts & " " & NoteNum
					
					set nts to nts & " </small></a>"
					if i = nn then
						set nts to nts & "]</sup>"
					end if
