(*	importing Individuals in Tab-Delimited File script
	...
	1. The first row lists GEDCOM tags for the data in that column. For 
		suborindate data the tags are in the format "BIRT.DATE"
	...
	3. Each subsequent row has data for a single individual
			...
			if recData is not "" then
				...
				if tag1 is "NAME" and numTags is 1 then
					...
					if useRecRef is not "" then
						...
						-- special case for SURN and GIVN
						if last item of tagList is "SURN" then
							tell useRecRef
								if recData does not contain "[*]" and recData is not "no name" then
									set name to "/" & recData & "/"
								else
									set name to "//"
								end if
							end tell
						else if last item of tagList is "GIVN" then
							tell useRecRef
								set oldname to evaluate expression "NAME"
								if oldname is "" or oldname is "Male /New/" then
									set name to recData & " //"
								else
									set name to recData & " " & oldname
								end if
							end tell
						end if
					end if
