(*	Create Web site script - in Swedish
...
property scriptName : "Basic Web Site"
property rootName : "GC Site"
property eventOut : {"BAPM", "CHR", "CENS", "EMMI", "IMMI", "NATU", "BURI"}
property eventName : {"Baptism", "Kristning", "Census", "Emmigration", "Immigration", "Naturalisation", "Begravning"}
property eventVerb : {"var döpt", "var kristnad", "är i census från år", "emmigraterad", "immigraterad", "var naturaliserad", "var begravd"}
property attrOut : {"RESI", "DSCR", "EDUC", "NATI", "OCCU", "RELI", "TITL"}
property attrName : {"Lägenhet", "Beskrivning", "Education", "Nationalitet", "Yrke", "Religion", "Adeltitel
"}
...
set end of NameList to "<tr><th></th><th>Namn</th><th>Livstid</th><th>Född</th><th>Dog</th></tr>" & return
set namekey to "<b>Nyckel: </b>"
set keynum to 0
set currentLet to ""
...
tell application "GEDitCOM II"
	...
	set prevIndi to my GetHTMLName(item num of recs, "I")
	set currentIndi to my GetHTMLName(item 1 of recs, "I")
	set nextIndi to my GetHTMLName(item 2 of recs, "I")
	repeat with i from 1 to num
		set indiRec to item i of recs
		...
		if i < num - 1 then
			set nextIndi to my GetHTMLName(item (i + 2) of recs, "I")
		else
			set nextIndi to my GetHTMLName(item 1 of recs, "I")
		end if
...
set end of itext to "<h1>" & fname & " Genealogi</h1>" & return
set end of itext to "<p>" & namekey & "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
set end of itext to "| <a href='#contact'>Kontakt</a> |</p>" & return
...
set end of itext to "<p><a name='contact'></a>Genealogi data kontakt: " & msname
if msemail is not "" then
	set end of itext to "<br>Email: <a href='mailto:" & msemail & "'>" & msemail & "</a>" & return
end if
if msphone is not "" then
	set end of itext to "<br>Tel: " & msphone & return
end if
set end of itext to "</p>" & return

-- GEDitCOM credit
set end of itext to "<hr /><p>Web sidan skapad med Macintosh genealogiprogram "
set end of itext to "<a href='http://www.geditcom.com'>GEDitCOM II</a><br>" & return
set end of itext to "(geditcom.com är inte ansvarig för innehållet av denna web sida)"
set end of itext to "</p>" & return
...
write (itext as string) to inum as «class utf8»
close access inum
...
on CheckAvailable(sName)
	tell application "GEDitCOM II"
		if versionNumber < 1.09 then
			display dialog "Denna skripten fordrar GEDitCOM II, Version 1.1 eller nyare. Uppgradera och försök igen, var så god." buttons {"OK"} default button "OK" with title sName
			return false
		end if
		if number of documents is 0 then
			display dialog "Ni borde  öppna documenten i GEDitCOM II för att använda denna scripten" buttons {"OK"} default button "OK" with title sName
			return false
		end if
	end tell
	return true
...
on CreateSiteFolder(root)
	...
	set pickPrompt to "Välja mappen där den nya '" & root & "' mappen skall bli lagrad"
	...
	on error
		tell application "GEDitCOM II"
			beep
			display dialog "Kunde inte skapa mappen för websidan. " & ¬
				"Du kanske har inte skrivrättigheter till detta läge, " & ¬
				"läget är fult eller något annat diskfel hände. Välja ett nytt läge, var så god."
			return false
		end tell
	end try
	...
	write CSSText() to refnum as «class utf8»
	close access refnum
...
on ExportPage(indiRec)
	...
	set contnts to contnts & "<center><b>Innehåll</b></center>" & return
	...
	*)
	set contnts to contnts & "<li><a href='#faminfo'>Person- och Familjeinformation</a></li>" & return
	set end of itext to "<h2><a name='faminfo'></a>Person- och Familjeinformation</h2>" & return
	set HeSheNow to CurrentName
	
	-- birth and death
	set LastPlace to ""
	set pref to "<p>"
	set etext to GetTextForEvent(indiRec, "BIRT", "föddes", pref, "")
	...
	set LastPlace to ""
	set etext to GetTextForEvent(indiRec, "DEAT", "dog", pref, "")
	if etext is not "" then
		set pref to ""
		set end of itext to etext
		if last character of etext is return then
	...
	if FatherRef is not "" or MotherRef is not "" then
		set contnts to contnts & "<li><a href='#pedi'>Pedigree antavla</a></li>" & return
		set end of itext to "<h2><a name='pedi'></a>Pedigree antavla (3 generationer)</h2><br clear='all'>" & return
	...
	if evnts is not "" then
		set contnts to contnts & "<li><a href='#evnts'>Händelser</a></li>" & return
		set end of itext to "<h2><a name='evnts'></a>Händelser</h2>" & return
	...
	if attrs is not "" then
		set contnts to contnts & "<li><a href='#attrs'>Egenskaper</a></li>" & return
		set end of itext to "<h2><a name='attrs'></a>Egenskaper</h2>" & return
		set end of itext to "<table id='attr'>" & return
		set end of itext to "<tr><th>Egenskap</th><th>Detaljer</th></tr>" & return
	...
	if nts is not "" then
		set contnts to contnts & "<li><a href='#notes'>Noter</a></li>" & return
		set end of itext to "<h2><a name='notes'></a>Noter</h2>" & return
	...
	write pagetext to inum as «class utf8»
	close access inum
end ExportPage
...
on GetIndiPageIndex()
	...
	set indbar to indbar & "<li><a href='../index.html'>Namnförteckning</a></li> " & return
	set indbar to indbar & "<li><a href='" & prevIndi & "'>Föregående</a></li> " & return
	set indbar to indbar & "<li><a href='" & nextIndi & "'>Följande</a></li> " & return
			...
			tell evnt
				...
				if LastPlace is "" then
					...
					if ed is "" then
						...
						set ed to ed & " i " & LastPlace & ". "
					end if
				end if
				
				-- if no date and place, say so
				if ed is "" then
					set ed to HeSheNow & "  " & everb & ", men datum och platsen är okända. "
				end if
...
on GetParents(indiRec)
	tell application "GEDitCOM II"
		...
		repeat with f from 1 to numPar
			...
			set par to HeSheNow & " var " & chil & " till "
			if fath is "" then
				if moth is "" then
					set par to par & "okända föräldrar. "
				else
					set par to par & " en okänd fader och " & my GetLinkTo(moth, "") & ". "
				end if
			else
				if moth is "" then
					set par to par & my GetLinkTo(fath, "") & " och en okänd moder. "
				else
					set par to par & my GetLinkTo(fath, "") & " och " & my GetLinkTo(moth, "") & ". "
				end if
			end if
...
on GetMarriages(indiRec)
	...
	tell application "GEDitCOM II"
		...
		set mtext to "<p>"
		if nmr > 1 then
			set mtext to mtext & HeSheNow & " hade " & nmr & " äktenskap/partners. "
			set HeSheNow to HeShe
		...
		repeat with m from 1 to nmr
			...
			if spse is not "" then
				set mtext to mtext & " var " & my GetLinkTo(spse, "") & ". "
			else
				set mtext to mtext & " är ej känd. "
			end if
			
			-- marriage details
			if umr is not "Y" then
				set HeSheNow to "De"
				set mtext to mtext & my GetTextForEvent(famRec, "MARR", "vigdes", "", "")
				if last character of mtext is return then
					set mtext to mtext & "<p>"
				end if
				set HeSheNow to HeShe
			else
				set mtext to mtext & "Vigdes aldrig. "
			end if
			
			-- list all children or say none
			set nch to number of items in chils
			if nch = 0 then
				set mtext to mtext & "De hade inga kända barn"
			else
				if nch = 1 then
					set mtext to mtext & "Deras kända barn var "
				else
					set mtext to mtext & "Deras " & nch & " kända barn var "
				end if
				repeat with c from 1 to nch
					set chilID to contents of item c of chils
					if c > 1 then
						if c = nch then
							set mtext to mtext & " och "
						else
							set mtext to mtext & ", "
						end if
					end if
...
on CopyOBJEFile(objeRef)
	tell application "GEDitCOM II"
		...
		if mmLoc is not "" or theDate is not "" then
			set mmtext to mmtext & "<p>Den multimedia objecten var i "
			if mmLoc is "" then
		...
		if movieTypes contains mmForm then
			set mmtext to mmtext & "<a href='" & mmName & "'>Click att se movie</a>" & return
		else
	...
	write mmtext to onum as «class utf8»
	close access onum
