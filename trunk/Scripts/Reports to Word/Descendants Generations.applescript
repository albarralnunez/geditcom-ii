(*	Descendants Generations To Word Script
	...
	"Reports" section or in the "Reports to Pages" section
...
on WriteToReport(indvRef, DList)
	...
	tell application "Microsoft Word"
		make document
		activate
		set titleFont to {all caps:true, bold:true, italic:false, font size:14, name:"Times New Roman"}
		set titlePara to {alignment:align paragraph center}
		set subTitleFont to {all caps:false, bold:false, font size:12}
		set subTitlePara to ""
		...
		set genFont to {italic:true, font size:14}
		set genPara to {first line indent:0, paragraph format left indent:0, alignment:align paragraph left, space before:12, space after:3}
		set indiNumFont to {bold:true, italic:false, font size:12}
		set indiNumPara to ""
		set indiFont to {bold:false, italic:false, font size:12}
		set indiPara to {first line indent:-15, paragraph format left indent:15, alignment:align paragraph justify, space before:9, space after:0}
		set parentFont to {italic:false, font size:12}
		set parentPara to {first line indent:0, paragraph format left indent:15, space before:4, space after:2}
		set childFont to {italic:true, font size:10}
		set childPara to {first line indent:-15, paragraph format left indent:60, space before:0, space after:1}
		...
		tell first section of front document
			...
			-- insert page number, but requires Word 2008
			--tell hf
			--	create new field text range text object field type field page
			--end tell
			my TextToWord(tab & "Descendants of " & fname & tab, hfFont, "")
	...
	TextToWord("Descendants of " & fname & return & return, titleFont, titlePara)
	TextToWord("Report prepared: " & (current date) & return, subTitleFont, subTitlePara)
	TextToWord("Number of generations found: " & (numDList - 1) & return, "", "")
	...
	repeat with i from 1 to numDList
		...
		TextToWord("Generation No. " & (i - 1) & return, genFont, genPara)
		...
		repeat while j ² numGen
			...
			tell application "GEDitCOM II"
				tell thisIndi
					...
					my TextToWord((dNum as string), indiNumFont, indiNumPara)
					my TextToWord(". ", indiFont, indiPara)
					if linkNum is "" then
						...
						repeat while i < numExtraEvents
							...
							try
								...
								if etext is not "" then
									my TextToWord(pronoun & etext & ". ", "", "")
								end if
							end try
							set i to i + 3
						end repeat
					else
						my TextToWord(alternate name & " (see #" & linkNum & ")", "", "")
			...
			repeat with jj from 1 to (count of fams)
				...
				set spouseDes to ""
				if spouse is "" then
					my TextToWord(HeShe & " married an unknown spouse", "", "")
				else
					tell application "GEDitCOM II"
						tell spouse
							my TextToWord(HeShe & " married " & alternate name, "", "")
					...
					tell application "GEDitCOM II"
						...
						tell fam
							...
							if numChil > 0 then
								...
								set end of cpt to parentFont
								set end of cpt to parentPara
							end if
						end tell
						...
						if numChil > 0 then
							repeat with ic from 1 to numChil
								...
								tell ichil
									...
									-- style
									if ic is 1 then
										set end of cpt to childFont
										set end of cpt to childPara
									else
										set end of cpt to ""
										set end of cpt to ""
									end if
			...
			repeat with ic from 1 to count of cpt by 3
				my TextToWord(item ic of cpt, item (ic + 1) of cpt, item (ic + 2) of cpt)
			end repeat
